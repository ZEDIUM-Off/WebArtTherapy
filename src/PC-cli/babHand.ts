...
export function initHand(
  keys: mpHands.NormalizedLandmarkList[],
  index: number,
  side: string,
) {
  ...
  for (let i = 0; i < keypoints.length; i++) {
    ...
    const sphere = BABYLON.MeshBuilder.CreateSphere(
      name,
      { diameter: 0.1, updatable: true, segments: 1 },
      BENV.scene,
    );
    if (sphere != null) {
...
function moveHand(
  ...
  index: number,
  side: string,
  pos: mpHands.NormalizedLandmarkList[],
) {
  let keypoints: mpHands.NormalizedLandmark[] = [];
  ...
  for (let i = 0; i < keypoints.length; i++) {
    ...
    if (sphere != null) {
      sphere.position.set(
        2.5 + -5 * keypoints[i].x + posTab[0].x * -5,
        4 + -5 * keypoints[i].y + posTab[0].y * -5,
        5 * keypoints[i].z,
      );
    }
  }
...
// remove hand
function removeHand(
  keyTab: mpHands.NormalizedLandmarkList[],
  handTab: mpHands.Handedness[],
) {
  let name;
...
function actiontHand(
  keyTab: mpHands.NormalizedLandmarkList[],
  handTab: mpHands.Handedness[],
  posTab: mpHands.NormalizedLandmarkList[],
) {
...
function pinched(
  posThumb: mpHands.NormalizedLandmark,
  posIndex: mpHands.NormalizedLandmark,
) {
...
function grab(
  ...
  if (box != null && thumb != null) {
    const distance = Math.sqrt(
      Math.pow(thumb.position.x - box.position.x, 2) +
        Math.pow(thumb.position.y - box.position.y, 2) +
        Math.pow(thumb.position.z - box.position.z, 2),
    );
    if (pinched(posThumb, posIndex)) {
      if (
        distance < 0.3 &&
        thumb.physicsImpostor != null &&
        box.physicsImpostor != null &&
        !joined
      ) {
